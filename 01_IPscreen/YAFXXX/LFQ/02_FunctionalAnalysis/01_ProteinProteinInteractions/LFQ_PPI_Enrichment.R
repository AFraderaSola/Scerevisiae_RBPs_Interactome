###############################################################################
#
# This script generates the functional analys (GO, KEGG and Reactome) for the
# LFQ results included in the RBP interactome project 
#
...
pg_quant_file <- list.files(path = "../../01_CoreAnalysis/20220429_results_LFQ/",pattern = ".*QuantifiedProteins.csv")

# Define your enriched protein groups path and name:

pg_enriched_file <- list.files(path = "../../01_CoreAnalysis/20220429_results_LFQ/",pattern = ".*Enriched_RNase_vs_WT.csv")
...
genename <- paste0(masterfile[masterfile$Stock_Name == gsub(pattern = ".*een/|/LFQ.*",replacement = "",x = getwd()),]$Gene_Name, "-TAP")
...
for (j in 1:length(direction_user)) {
  ...
  for (i in 1:length(ont_user)) {
    ...
    pg_quant <- read_csv(paste0("../../01_CoreAnalysis/20220429_results_LFQ/",pg_quant_file))
    ...
    pg_enriched <- read_csv(paste0("../../01_CoreAnalysis/20220429_results_LFQ/",pg_enriched_file))
