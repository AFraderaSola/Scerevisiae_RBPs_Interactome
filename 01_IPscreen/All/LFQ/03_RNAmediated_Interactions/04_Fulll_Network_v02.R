...
E(net)$edge.color <- ifelse(E(net)$edge.color == "Metabolism", "#E5C494", E(net)$edge.color)
...
RDI_All_vis_nodes <- vertices

colnames(RDI_All_vis_nodes) <- c("id", "n", "Functionality")

RDI_All_vis_edges <- edges

colnames(RDI_All_vis_edges) <- c("from", "to", "Functionality")

file <- paste0("./05_OutputFiles/All_RDI_InteractiveNetwork_v02.RData")

save(RDI_All_vis_nodes, RDI_All_vis_edges, file = file)
